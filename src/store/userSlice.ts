...
export const updateUserAsync = createAsyncThunk(
    ...
    async (userData: { id: number; email: string; first_name: string; last_name: string }, { rejectWithValue }) => {
        try {
            const response = await api.users.usersUpdate(userData);
            return response.data;
        } catch (error) {
            const axiosError = error as AxiosError<ApiError>;
            return rejectWithValue(axiosError.response?.data?.error || 'Ошибка при обновлении данных');
        }
    }
);
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(loginUserAsync.fulfilled, (state, action) => {
                const { id, email, firstName, lastName, is_staff } = action.payload;
                state.id = id;
                state.email = email;
